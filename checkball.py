...
api_cache = TTLCache(maxsize=1000, ttl=120)  # 2 minute TTL
...
class SportsAPI:
    ...
    def get_scores(self, sport, team_name):
        ...
        try:
            ...
            # Fetch games from past 3 days and next 5 days for better coverage
            for days_offset in [-3, -2, -1, 0, 1, 2, 3, 4, 5]:
    ...
    def _find_team_game(self, sport, team_name, base_url):
        """Find the most relevant game for a team - prioritizes live/today's games"""
        try:
            # Get current time in EST/EDT
            eastern = pytz.timezone('US/Eastern')
            now = datetime.now(eastern)

            # Collect ALL games first, then prioritize
            all_games = []

            # Search today first, then other days
                    ...
                    for game in data.get('events', []):
                        ...
                        for team in competitors:
                            ...
                            if self._team_name_matches(team_name, team_display_name):
                                ...
                                break  # Found team in this game, move to next game
            ...
            # Log all found games for debugging
            logger.info(f"Found {len(all_games)} games for {team_name}")
            for g in all_games:
                comp = g['game'].get('competitions', [{}])[0]
                teams_str = f"{comp.get('competitors', [{}])[1].get('team', {}).get('displayName', 'Unknown')} @ {comp.get('competitors', [{}])[0].get('team', {}).get('displayName', 'Unknown')}"
                logger.info(f"  {g['date']} - {teams_str} - {g['status'].get('name', 'Unknown')}")

            # Sort ALL games by date (most recent first)
            all_games.sort(key=lambda x: x['date'], reverse=True)

            # Prioritize: live games > most recent completed > any game
            live_games = [g for g in all_games if g['status'].get('name') in ['STATUS_IN_PROGRESS', 'STATUS_HALFTIME', 'STATUS_END_PERIOD']]
            completed_games = [g for g in all_games if g['status'].get('completed', False)]

            logger.info(f"After sorting: {len(live_games)} live, {len(completed_games)} completed")
            if completed_games:
                comp = completed_games[0]['game'].get('competitions', [{}])[0]
                winner = f"{comp.get('competitors', [{}])[1].get('team', {}).get('displayName', 'Unknown')} @ {comp.get('competitors', [{}])[0].get('team', {}).get('displayName', 'Unknown')}"
                logger.info(f"Returning most recent completed: {completed_games[0]['date']} - {winner}")

            # Return in priority order
            if live_games:
                # Return most recent live game
                return live_games[0]
            if completed_games:
                # Return most recently completed game (already sorted by date)
                return completed_games[0]

            # Fallback: return most recent game overall
            return all_games[0]
...
class GamePredictor:
    """Predictive model for game outcomes using live ESPN data"""

    def __init__(self, sports_api_instance):
        self.api = sports_api_instance
        self.cache = TTLCache(maxsize=100, ttl=3600)  # 1 hour cache for stats

    def get_team_recent_stats(self, sport, team_name, num_games=10):
        """Get recent game statistics for a team"""
        cache_key = f"stats:{sport}:{team_name}:{num_games}"
        ...
        try:
            ...
            games = []
            base_url = self._get_sport_base_url(sport)
            ...
            for days_offset in range(-30, 0):
                ...
                try:
                    data = self.api._make_api_request(url, timeout=3)
                    for game in data.get('events', []):
                        if game.get('status', {}).get('type', {}).get('completed', False):
                            competitors = game.get('competitions', [{}])[0].get('competitors', [])
                            for team in competitors:
                                if self.api._team_name_matches(team_name, team.get('team', {}).get('displayName', '')):
                                    games.append(self._parse_game_stats(game, team, sport))
                                    break
                except:
                    continue
    ...
    def _get_sport_base_url(self, sport):
        """Get ESPN API base URL for sport"""
        sport_map = {
            'nba': 'basketball/nba',
        ...
        return f"{self.api.base_url}/{sport_path}"

    def _parse_game_stats(self, game, team_data, sport):
        """Parse statistics from a game"""
        stats = {
            'score': int(team_data.get('score', 0)),
            'won': team_data.get('winner', False),
            'home': team_data.get('homeAway', 'away') == 'home'
        }
    ...
    def _aggregate_stats(self, games, sport):
        ...
        stats = {
            ...
            'avg_score': np.mean([g['score'] for g in games]),
            'win_rate': sum(1 for g in games if g['won']) / len(games),
            'home_win_rate': sum(1 for g in games if g['won'] and g['home']) / sum(1 for g in games if g['home']) if any(g['home'] for g in games) else 0.5,
            'away_win_rate': sum(1 for g in games if g['won'] and not g['home']) / sum(1 for g in games if not g['home']) if any(not g['home'] for g in games) else 0.5,
        }

        # Sport-specific statistics
        if sport.lower() in ['nba', 'wnba']:
            stats.update({
                'avg_fg_pct': np.mean([float(g.get('fieldGoalPct', 0)) for g in games if 'fieldGoalPct' in g]),
                'avg_3pt_pct': np.mean([float(g.get('threePointFieldGoalPct', 0)) for g in games if 'threePointFieldGoalPct' in g]),
                'avg_rebounds': np.mean([float(g.get('rebounds', 0)) for g in games if 'rebounds' in g]),
                'avg_assists': np.mean([float(g.get('assists', 0)) for g in games if 'assists' in g]),
            })
    ...
    def predict_game_outcome(self, sport, team1, team2, location='neutral'):
        ...
        try:
            ...
            # Factor 1: Recent form (win rate) - most important
            form_diff = team1_stats['win_rate'] - team2_stats['win_rate']
            factors.append(form_diff * 35)  # Weight: 35%

            # Factor 2: Scoring differential (points for - points against implied)
            # Higher scoring team has advantage, but weighted against opponent's scoring
            team1_score_advantage = team1_stats['avg_score'] - team2_stats['avg_score']
            score_factor = (team1_score_advantage / 10) * 30  # Weight: 30%
            factors.append(score_factor)

            # Factor 3: Home/Away performance (more nuanced)
            if location == 'team1_home':
                home_boost = (team1_stats['home_win_rate'] - 0.5) * 15  # Up to 7.5% boost
                factors.append(home_boost + 3)  # Base 3% + performance-based
            elif location == 'team2_home':
                home_boost = (team2_stats['home_win_rate'] - 0.5) * 15
                factors.append(-(home_boost + 3))

            # Factor 4: Sport-specific metrics (NBA/WNBA)
            if sport.lower() in ['nba', 'wnba']:
                # Field goal percentage is highly correlated with wins
                fg_diff = team1_stats.get('avg_fg_pct', 0) - team2_stats.get('avg_fg_pct', 0)
                factors.append(fg_diff * 25)  # Weight: 25%

                # Rebounds and assists (smaller weight)
                ...
                factors.append((reb_diff / 10) * 5)  # Weight: 5%
                factors.append((ast_diff / 5) * 5)   # Weight: 5%

            # Calculate final probability
            total_advantage = sum(factors)
            team1_win_prob = 50 + total_advantage
            team1_win_prob = max(20, min(80, team1_win_prob))  # Clamp between 20-80%

            # IMPROVED: Predicted scores that strongly align with win probability
            # Start with team averages but heavily weight toward probability
            team1_base_score = team1_stats['avg_score']
            team2_base_score = team2_stats['avg_score']

            # Calculate what the score margin SHOULD be based on win probability
            # Higher win prob = bigger predicted margin
            prob_diff = team1_win_prob - 50  # Positive if team1 favored

            # Convert probability advantage to expected point differential
            # NBA games: 10% win prob advantage ≈ 3 point margin
            # 20% advantage ≈ 6-7 points, 30% advantage ≈ 10+ points
            expected_margin = prob_diff * 0.35  # 0.35 points per 1% probability

            # Calculate average total score
            avg_total_score = (team1_base_score + team2_base_score) / 2

            # Apply home court adjustments to the margin
            home_advantage_points = 0
            if location == 'team1_home':
                home_advantage_points = 3  # Team1 gets +3 points at home
            elif location == 'team2_home':
                home_advantage_points = -3  # Team2 gets +3 points at home

            # Final predicted margin (team1 score - team2 score)
            final_margin = expected_margin + home_advantage_points

            # Calculate individual scores while maintaining the margin
            # Keep close to team averages but ensure proper winner
            if final_margin > 0:
                # Team1 should win
                team1_predicted_score = avg_total_score + (final_margin / 2)
                team2_predicted_score = avg_total_score - (final_margin / 2)
            else:
                # Team2 should win
                team1_predicted_score = avg_total_score + (final_margin / 2)
                team2_predicted_score = avg_total_score - (final_margin / 2)

            # Blend with actual team scoring averages (70% probability-based, 30% averages)
            team1_predicted_score = (team1_predicted_score * 0.7) + (team1_base_score * 0.3)
            team2_predicted_score = (team2_predicted_score * 0.7) + (team2_base_score * 0.3)

            # Final safety check: ensure predicted winner matches probability winner
            if team1_win_prob > 50 and team1_predicted_score <= team2_predicted_score:
                # Force team1 to have higher score
                diff = (team2_predicted_score - team1_predicted_score) + 3
                team1_predicted_score += diff / 2
                team2_predicted_score -= diff / 2
            elif team1_win_prob < 50 and team1_predicted_score >= team2_predicted_score:
                # Force team2 to have higher score
                diff = (team1_predicted_score - team2_predicted_score) + 3
                team2_predicted_score += diff / 2
                team1_predicted_score -= diff / 2
            ...
            return {
                ...
                'confidence': 'High' if abs(team1_win_prob - 50) > 20 else 'Medium' if abs(team1_win_prob - 50) > 10 else 'Low',
                'location': location
            }
...
# Initialize sports API and predictor
sports_api = SportsAPI()
game_predictor = GamePredictor(sports_api)
...
@limiter.limit("10 per minute")  # Rate limit predictions
def predict_game():
    """Predict game outcome between two teams"""
    try:
        data = request.json
        ...
        location = data.get('location', 'neutral')

        # Security: Sanitize inputs
        sport = sanitize_input(sport, VALID_SPORT_PATTERN, max_length=50)
        team1 = sanitize_input(team1, VALID_TEAM_PATTERN, max_length=100)
        team2 = sanitize_input(team2, VALID_TEAM_PATTERN, max_length=100)

        if not sport or not team1 or not team2:
            return jsonify({'error': 'Invalid parameters'}), 400

        prediction = game_predictor.predict_game_outcome(sport, team1, team2, location)
...
@app.route('/api/upcoming-games/<path:sport>/<path:team>')
@limiter.limit("15 per minute")
def get_upcoming_games(sport, team):
    """Get upcoming games for a team with prediction data"""
    import urllib.parse
    sport = urllib.parse.unquote(sport)
    team = urllib.parse.unquote(team)
    ...
    try:
        ...
        base_url = game_predictor._get_sport_base_url(sport)
        ...
        for days_offset in range(0, 7):
            ...
            try:
                data = sports_api._make_api_request(url, timeout=3)
                for game in data.get('events', []):
                    status = game.get('status', {}).get('type', {}).get('name', '')
                    if status == 'STATUS_SCHEDULED':
                        competitors = game.get('competitions', [{}])[0].get('competitors', [])
                        for team_data in competitors:
                            if sports_api._team_name_matches(team, team_data.get('team', {}).get('displayName', '')):
                                opponent = [c for c in competitors if c['id'] != team_data['id']][0]
                                opponent_name = opponent.get('team', {}).get('displayName', '')
