...
function updateScoreDisplay(widgetIndex, data) {
    ...
    if (data.next_game && nextGameElement && !isUpcoming) {
        ...
        const myTeam = userConfig[`widget_${widgetIndex}`]?.team;
        ...
        // Make it clickable to show upcoming game stats and prediction
        nextGameElement.onclick = function() {
            openUpcomingGameModal(sport, myTeam, nextGame.opponent, nextGame.game_date);
        };

        // Add visual indicator that it's clickable
        nextGameElement.style.cursor = 'pointer';
...
function displayDetailedGameData(detailedData, sport, widgetIndex) {
    ...
    modalContent.innerHTML = `
        <div class="game-matchup-scores">
            <div class="team-score-card">
                <div class="team-info-compact">
                    <div class="team-name-compact">${detailedData.away_team.name}</div>
                    <div class="team-abbreviation">${detailedData.away_team.short_name}</div>
                </div>
                <div class="score-large">${detailedData.away_team.score}</div>
            </div>

            <div class="game-status-center">
                <div class="status-badge ${isLive ? 'live' : isCompleted ? 'final' : 'upcoming'}">
                    ${isLive ? '🔴 LIVE' : isCompleted ? '✅ FINAL' : '📅 SCHEDULED'}
                </div>
                <div class="game-time">${gameDateTime}</div>
                ${detailedData.venue?.name ? `<div class="venue-info">📍 ${detailedData.venue.name}</div>` : ''}
            </div>

            <div class="team-score-card">
                <div class="team-info-compact">
                    <div class="team-name-compact">${detailedData.home_team.name}</div>
                    <div class="team-abbreviation">${detailedData.home_team.short_name}</div>
                </div>
                <div class="score-large">${detailedData.home_team.score}</div>
            </div>
        </div>
        ...
        <div class="modal-actions">
            <button onclick="predictGame('${sport}', '${detailedData.away_team.name}', '${detailedData.home_team.name}', 'team2_home')"
                    class="action-btn primary-btn"
                    style="background: linear-gradient(135deg, #8b5cf6, #6366f1); box-shadow: 0 0 25px rgba(139, 92, 246, 0.4);">
...
function createPredictionModal() {
    ...
    const modalHTML = `
        <div id="predictionModal" class="prediction-modal">
            <div class="prediction-modal-content">
                <span class="prediction-modal-close">&times;</span>
                <div id="predictionModalContent">
                    <div class="loading-spinner">
                        <div class="spinner-icon">🔮</div>
                        <p>Analyzing team statistics...</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    closeBtn.onclick = closePredictionModal;

    predictionModal.onclick = function(event) {
    ...
    document.addEventListener('keydown', function(event) {
        if (event.key === 'Escape' && predictionModal && predictionModal.style.display === 'block') {
            closePredictionModal();
        }
    });
}
...
async function predictGame(sport, team1, team2, location) {
    ...
    try {
        modalContent.innerHTML = `
            <div class="loading-spinner">
                <div class="spinner-icon">🔮</div>
                <p>Analyzing team statistics and recent performance...</p>
                <div class="loading-progress"></div>
            </div>
        `;
        ...
        displayPrediction(prediction, sport);

    } catch (error) {
        console.error('Error getting prediction:', error);
        modalContent.innerHTML = `
            <div class="prediction-header">
                <h2>⚠️ Prediction Unavailable</h2>
                <p class="prediction-subtitle">${error.message}</p>
            </div>
            <div class="error-message" style="text-align: center; padding: 2rem;">
                <p>Unable to generate prediction. This could be due to:</p>
                <ul style="text-align: left; max-width: 400px; margin: 1rem auto;">
                    <li>Insufficient recent game data</li>
                    <li>Teams haven't played enough games this season</li>
...
function displayPrediction(prediction, sport) {
    const modalContent = document.getElementById('predictionModalContent');
    ...
    modalContent.innerHTML = `
        <div class="prediction-header">
            ...
            <p class="prediction-subtitle">${team1.name} vs ${team2.name}</p>
        </div>

        <div class="probability-container">
            <div class="team-probability">
                <div class="probability-label">
                    <span class="probability-team-name">${escapeHtml(team1.name)}</span>
                    <span class="probability-value">${team1.win_probability}%</span>
                </div>
                <div class="probability-bar">
                    <div class="probability-fill" style="width: ${team1.win_probability}%"></div>
                </div>
                <div style="margin-top: 0.5rem; font-size: 0.9rem; color: rgba(255,255,255,0.7);">
                    Recent Form: ${team1.recent_form} • Avg: ${team1.avg_score} pts
                </div>
            </div>

            <div class="team-probability">
                <div class="probability-label">
                    <span class="probability-team-name">${escapeHtml(team2.name)}</span>
                    <span class="probability-value">${team2.win_probability}%</span>
                </div>
                <div class="probability-bar">
                    <div class="probability-fill" style="width: ${team2.win_probability}%"></div>
                </div>
                <div style="margin-top: 0.5rem; font-size: 0.9rem; color: rgba(255,255,255,0.7);">
                    Recent Form: ${team2.recent_form} • Avg: ${team2.avg_score} pts
                </div>
            </div>
        </div>

        <div class="predicted-scores">
            <div class="predicted-score-card">
                <div class="predicted-score-label">${escapeHtml(team1.name)}</div>
                <div class="predicted-score-value">${team1.predicted_score}</div>
            </div>

            <div class="predicted-vs">VS</div>

            <div class="predicted-score-card">
                <div class="predicted-score-label">${escapeHtml(team2.name)}</div>
                <div class="predicted-score-value">${team2.predicted_score}</div>
            </div>
        </div>

        <div class="confidence-indicator">
            <div class="confidence-badge ${prediction.confidence.toLowerCase()}">
        ...
        <div style="margin-top: 2rem; padding: 1.5rem; background: rgba(6, 182, 212, 0.1); border: 2px solid rgba(6, 182, 212, 0.3); border-radius: 14px;">
            <h4 style="color: var(--accent-neon); margin-bottom: 0.75rem; font-size: 1rem;">📈 Prediction Factors</h4>
            <ul style="color: rgba(255,255,255,0.8); font-size: 0.9rem; line-height: 1.8; list-style: none; padding: 0;">
                <li>✓ Based on last 10 games for each team</li>
                <li>✓ Recent form and win rate analysis</li>
                <li>✓ Average scoring performance</li>
                <li>✓ Home court advantage factored in</li>
                ${sport.toLowerCase() === 'nba' || sport.toLowerCase() === 'wnba' ? '<li>✓ Field goal % and shooting stats</li>' : ''}
            </ul>
        </div>

        <div class="modal-footer" style="margin-top: 2rem; padding-top: 1.5rem; border-top: 1px solid rgba(255,255,255,0.1);">
            <div class="data-source">AI-Powered Prediction • Live ESPN Data • Statistical Analysis</div>
        </div>
    `;
}
...
function getConfidenceExplanation(confidence) {
    const explanations = {
        'High': 'Strong statistical indicators suggest a clear favorite based on recent performance.',
        'Medium': 'Teams are relatively evenly matched. Could go either way based on game day performance.',
        'Low': 'Very close matchup. Statistics show minimal difference between teams.'
    };
    return explanations[confidence] || 'Prediction based on available statistical data.';
}

// ============================================
//  UPCOMING GAME MODAL - Next Game Analysis
// ============================================

let upcomingGameModal = null;

function createUpcomingGameModal() {
    if (upcomingGameModal) return;

    const modalHTML = `
        <div id="upcomingGameModal" class="prediction-modal">
            <div class="prediction-modal-content">
                <span class="upcoming-game-modal-close">&times;</span>
                <div id="upcomingGameModalContent">
                    <div class="loading-spinner">
                        <div class="spinner-icon">📅</div>
                        <p>Loading upcoming game analysis...</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    document.body.insertAdjacentHTML('beforeend', modalHTML);
    upcomingGameModal = document.getElementById('upcomingGameModal');
    const closeBtn = document.querySelector('.upcoming-game-modal-close');

    closeBtn.onclick = closeUpcomingGameModal;

    upcomingGameModal.onclick = function(event) {
        if (event.target === upcomingGameModal) {
            closeUpcomingGameModal();
        }
    };

    document.addEventListener('keydown', function(event) {
        if (event.key === 'Escape' && upcomingGameModal && upcomingGameModal.style.display === 'block') {
            closeUpcomingGameModal();
        }
    });
}
...
function closeUpcomingGameModal() {
    if (upcomingGameModal) {
        upcomingGameModal.style.display = 'none';
    }
}

async function openUpcomingGameModal(sport, myTeam, opponent, gameDate) {
    createUpcomingGameModal();
    upcomingGameModal.style.display = 'block';

    const modalContent = document.getElementById('upcomingGameModalContent');

    try {
        modalContent.innerHTML = `
            <div class="loading-spinner">
                <div class="spinner-icon">📊</div>
                <p>Analyzing teams and generating prediction...</p>
                <div class="loading-progress"></div>
            </div>
        `;

        // Fetch team statistics for both teams
        const [myTeamStatsResponse, opponentStatsResponse] = await Promise.all([
            fetch(`/api/game-details/${encodeURIComponent(sport)}/${encodeURIComponent(myTeam)}`),
            fetch(`/api/game-details/${encodeURIComponent(sport)}/${encodeURIComponent(opponent)}`)
        ]);

        const myTeamStats = await myTeamStatsResponse.json();
        ...
        const predictionResponse = await fetch('/api/predict', {
            ...
            body: JSON.stringify({
                ...
                team1: myTeam,
                team2: opponent,
                location: 'neutral' // We can enhance this later with actual home/away info
            })
        });
        ...
        displayUpcomingGameAnalysis(myTeam, opponent, myTeamStats, opponentStats, prediction, gameDate, sport);

    } catch (error) {
        console.error('Error loading upcoming game:', error);
        modalContent.innerHTML = `
            <div class="prediction-header">
            ...
            <div class="error-message" style="text-align: center; padding: 2rem;">
                <p>Unable to load upcoming game analysis. This could be due to:</p>
                <ul style="text-align: left; max-width: 400px; margin: 1rem auto;">
                    <li>Insufficient recent game data</li>
                    <li>API connection issue</li>
                    <li>Teams haven't played enough games</li>
                </ul>
            </div>
        `;
    }
...
function displayUpcomingGameAnalysis(myTeam, opponent, myTeamStats, opponentStats, prediction, gameDate, sport) {
    const modalContent = document.getElementById('upcomingGameModalContent');
    const team1 = prediction.team1;
    const team2 = prediction.team2;
    ...
    modalContent.innerHTML = `
        <div class="prediction-header">
            <h2>Upcoming Game Analysis</h2>
            <p class="prediction-subtitle">${escapeHtml(myTeam)} vs ${escapeHtml(opponent)}</p>
        ...
        ${myTeamStats.box_score && opponentStats.box_score ? `
            <div class="stats-section">
                <div class="stats-title">📊 Season Statistics Comparison</div>
                <div class="team-stats-grid">
                    ${generateUpcomingGameStatsComparison(myTeamStats, opponentStats, myTeam, opponent)}
                </div>
            </div>
        ` : ''}
        ...
        <!-- Prediction Factors -->
        <div style="margin-top: 2rem; padding: 1.5rem; background: rgba(6, 182, 212, 0.1); border: 2px solid rgba(6, 182, 212, 0.3); border-radius: 14px;">
            <h4 style="color: var(--accent-neon); margin-bottom: 0.75rem; font-size: 1rem;">📈 Analysis Factors</h4>
            <ul style="color: rgba(255,255,255,0.8); font-size: 0.9rem; line-height: 1.8; list-style: none; padding: 0;">
                <li>✓ Based on last 10 games for each team</li>
                <li>✓ Recent form and win rate trends</li>
                <li>✓ Average scoring performance</li>
                <li>✓ Head-to-head historical data</li>
                ${sport.toLowerCase() === 'nba' || sport.toLowerCase() === 'wnba' ? '<li>✓ Shooting efficiency and advanced metrics</li>' : ''}
            </ul>
        </div>

        <div class="modal-footer" style="margin-top: 2rem; padding-top: 1.5rem; border-top: 1px solid rgba(255,255,255,0.1);">
            <div class="data-source">AI-Powered Prediction • Live ESPN Data • Pre-Game Analysis</div>
        </div>
    `;
}

function generateUpcomingGameStatsComparison(team1Stats, team2Stats, team1Name, team2Name) {
    if (!team1Stats.box_score || !team2Stats.box_score) {
        return '<p style="text-align: center; color: rgba(255,255,255,0.6);">Statistics comparison unavailable</p>';
    }

    const team1Data = team1Stats.box_score[0] || {};
    const team2Data = team2Stats.box_score[0] || {};

    const stats = team1Data.stats || [];
    const team2StatsMap = {};

    // Create a map of team 2 stats for easy lookup
    if (team2Data.stats) {
        team2Data.stats.forEach(stat => {
            team2StatsMap[stat.name] = stat.displayValue;
        });
    }

    let html = `
        <div class="stats-grid-header">
            <div class="team-name-header">${escapeHtml(team1Name)}</div>
            <div class="stat-name-header">Statistic</div>
            <div class="team-name-header">${escapeHtml(team2Name)}</div>
        </div>
    `;

    // Display key stats
    const keyStats = ['fieldGoalPct', 'threePointFieldGoalPct', 'assists', 'rebounds', 'turnovers'];
    const statLabels = {
        'fieldGoalPct': 'FG%',
    ...
    stats.forEach(stat => {
        if (keyStats.includes(stat.name)) {
            const team2Value = team2StatsMap[stat.name] || '-';
            html += `
                <div class="stat-comparison-row">
                    <div class="stat-value-left">${stat.displayValue}</div>
                    <div class="stat-name-center">${statLabels[stat.name] || stat.name}</div>
                    <div class="stat-value-right">${team2Value}</div>
                </div>
            `;
        }
    });
